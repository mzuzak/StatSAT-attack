...
namespace ckt_n {
    ...
    {
        // std::cout<<"Entering _init_gate() in node.cpp\n";
        type = GATE;
        name = o;
        func = f;
        std::transform(func.begin(), func.end(), func.begin(), ::tolower);
        std::size_t found = name.find("$enc"); //Ankit
        if(found!=std::string::npos)
            keygate = true;
        //std::cout<<"Name: "<<name<<" func: "<<func<<"keygate = "<<keygate<<"\n";
        inputs.clear();
        fanouts.clear();
        output = false;
    }
    ...
    // Ankit - Calculate output error rate of a gate
    double node_t::calc_out_error_prob(std::vector<double>& p_in, std::vector<double>& eps_in, double err)
    {
        // From "Probabilistic error propagation in a logic circuit using the boolean difference calculus", 2011
        
        // p_in is the inputs' signal probabilities and eps_in is input error rate
        assert(p_in.size() == eps_in.size());
        assert(p_in.size() == inputs.size());

        err /= 100; // because error rates were in percentage

        if(func == "and" or func == "nand") 
        {
            double eps_ideal = eps_in[0]*p_in[1] + eps_in[1]*p_in[0] + eps_in[0]*eps_in[1]*(1 - 2*(p_in[0]+p_in[1]) + 2*p_in[0]*p_in[1]);
            double eps_out = err + (1-2*err)*eps_ideal;
            return eps_out;
        }
        else if(func == "or" or func == "nor")
        {
            double eps_ideal = eps_in[0]*(1-p_in[1]) + eps_in[1]*(1-p_in[0]) + eps_in[0]*eps_in[1]*(2*p_in[0]*p_in[1] - 1);
            double eps_out = err + (1-2*err)*eps_ideal;
            return eps_out;
        } 
        else if(func == "xor" or func == "xnor")
        {
            double eps_ideal = eps_in[0] + eps_in[1] - 2*eps_in[0]*eps_in[1] ;
            double eps_out = err + (1-2*err)*eps_ideal;
            return eps_out;
        }
        else if(func == "buf" or func == "not")
        {
            double eps_out = err + (1-2*err)*eps_in[0];
            return eps_out;
        }
        else{
            std::cout<<"Gate "<<name<<" function not one of and,nand,or,nor,buf,not,xor,xnor\n";
            exit(1);
        }
    }
