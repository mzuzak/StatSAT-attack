...
namespace AllSAT {
    ...
    int ClauseList::addRewrittenClauses(
        ...
        sat_n::Solver& S)
    {
        // std::cout<<"Entering addRewrittenClauses()A of ClauseList.cpp"<<std::endl;
        int cnt = 0;
        using namespace sat_n;
        std::map<Lit, Lit> rewrites;
        int count_def=0, count_undef=0;

        // std::cout<< "Num vars = "<<S.nVars()<<", clauses = "<<S.nClauses()
                    // <<" decisions = "<<S.getNumDecisions() << std::endl;

        // std::cout<<"No. of clauses = "<<numClauses()<<"\n";
        for(unsigned i=0; i != numClauses(); i++) 
        {
            int len = clauseLen(i);
            // if(i<10 || (numClauses()-i)<=10) // Ankit
            //     std::cout<<"Clause "<<i<<" has len = "<<len<<std::endl;
            assert(len > 0);
            vec_lit_t newClause;
            ...
            //}
            // if(i<10 || (numClauses()-i)<=10) // Ankit
            // dump_clause(std::cout << "original clause: ", i)  << std::endl; //Ankit
            ...
            for(int j=0; j != len; j++) 
            {
                ...
                {
                    count_def++;
                    // std::cout<<v_ij<<" is defined - "<<val_ij.getBool()<<" "<<sign(l_ij)<<"\n";
                    assert(val_ij.isDef());
        ...
        // std::cout<<"count_def = "<<count_def<<" and count_undef = "<<count_undef<<"\n";
        return cnt;
    }
    ...
    {
        std::cout<<"Entering addRewrittenClauses()B of ClauseList.cpp"<<std::endl;
        int cnt = 0;
        using namespace sat_n;
        std::map<Lit, Lit> rewrites;
